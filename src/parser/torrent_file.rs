...
impl TorrentFile {
    ...
    fn find_key_bytes_in_bencode(input_bencode: &BencodeValue, key: String) -> Result<Vec<u8> ,String> {
        match input_bencode {
    ...
    fn find_key_dict_in_bencode(input_bencode: &BencodeValue, key: String) -> Result<&BencodeValue ,String> {
        match input_bencode {
            BencodeValue::Dictionary(input) => {
                match input.get(&key.as_bytes().to_vec()) {
                    None => Err("Key not found".to_string()),
                    Some(value) => match value {
                        BencodeValue::Dictionary(bytes) => {
                            Ok(value)
                        }
                        _ => Err("Not a integer in the key".to_string()),
                    },
    ...
    fn find_key_integer_in_bencode(input_bencode: &BencodeValue, key: String) -> Result<i64, String> {
        match input_bencode {
            BencodeValue::Dictionary(input) => {
                match input.get(&key.as_bytes().to_vec()) {
                    None => Err("Key not found".to_string()),
    ...
    fn find_key_string_in_bencode(input_bencode: &BencodeValue, key: String) -> Result<String, String> {
        match input_bencode {
            BencodeValue::Dictionary(input) => {
                match input.get(&key.as_bytes().to_vec()) {
                    None => Err("Key not found".to_string()),
...
mod tests {
    ...
    fn find_string_in_bencode() {
        ...
        let result = TorrentFile::find_key_string_in_bencode(&input_bencode, "announce".to_string());
        assert!(result.is_ok());
    ...
    fn find_integer_in_bencode() {
        let mut input_map = HashMap::new();
        input_map.insert("length".as_bytes().to_vec(),BencodeValue::Integer("54".as_bytes().to_vec()));
        let input_bencode = BencodeValue::Dictionary(input_map);
